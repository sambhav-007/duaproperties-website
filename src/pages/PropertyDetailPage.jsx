        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {property.images_gallery.map((img, index) => (
              <img key={index} src={img} alt={`${property.name} gallery ${index + 1}`} className="w-full h-48 object-cover rounded-lg shadow-sm cursor-pointer transition-transform duration-300 hover:scale-105" />
            ))}
          </div>
        </div>
      )}
